		...
		<div id="list-artist" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="stageName" title="${message(code: 'artist.stageName.label', default: 'Stage Name')}" />
					
						<g:sortableColumn property="realName" title="${message(code: 'artist.realName.label', default: 'Real Name')}" />
				...
				<g:each in="${artistInstanceList}" status="i" var="artistInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${artistInstance.id}">${fieldValue(bean: artistInstance, field: "stageName")}</g:link></td>
					
						<td>${fieldValue(bean: artistInstance, field: "realName")}</td>
